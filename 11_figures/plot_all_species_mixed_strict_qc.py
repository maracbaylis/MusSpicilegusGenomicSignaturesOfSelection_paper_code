#!/usr/bin/env python3
"""Plot mixed strict QC: domesticus threshold 3, other species threshold 2."""
...
SUMMARY = Path("all_species_mixed_strict_fasta_qc_by_timepoint.tsv")
TOTALS = Path("frontal_cortex_foreground_totals.tsv")
OUT_TABLE = Path("all_species_mixed_strict_fasta_qc_percent_of_total.tsv")
OUT_PLOT = Path("all_species_mixed_strict_pass_percent_of_total_by_timepoint.png")
...
def main() -> None:
    ...
    df["mixed_strict_pass_percent_total"] = df["MIXED_STRICT_PASS"] / df["total_tested"] * 100
    ...
    for species in SPECIES_LABELS.values():
        ...
        ax.plot(
            TIMEPOINTS,
            sub["mixed_strict_pass_percent_total"],
            marker="o",
            linewidth=2,
            label=species,
        )
    ax.set_xlabel("Timepoint")
    ax.set_ylabel("Mixed strict PASS (% of total tested)")
    ax.set_title("Frontal Cortex Mixed Strict QC-confirmed Foreground Hits")
